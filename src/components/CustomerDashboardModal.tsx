...
import { X, User, ShoppingBag, CheckCircle, Clock, FileText, ExternalLink, MessageCircle, LogOut, ArrowRight, ShieldCheck, AlertCircle, Sparkles } from 'lucide-react';
...
export const CustomerDashboardModal: React.FC<CustomerDashboardModalProps> = ({
  ...
  const { user: firebaseUser, signInWithGoogle, logout: fbLogout } = useAuth();
  ...
  useEffect(() => {
    ...
    if (saved && saved.email) {
      ...
      setSession({ email: firebaseUser.email, name: firebaseUser.displayName || 'Customer' });
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (user && user.email) {
        ...
        const res = await customerLogin(user.email);
